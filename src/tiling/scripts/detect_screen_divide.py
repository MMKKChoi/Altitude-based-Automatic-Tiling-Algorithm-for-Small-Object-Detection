...
def split_image_with_overlap(image, rows, cols, overlap): # image, rows, cols, overlap
    ...
    for i in range(rows): # to divde the image into each grid, rotate the repetition statement for each row and col
        for j in range(cols):
            ...
            #split_images.append(image[y1:y2, x1:x2]) # add divided image to list
            #split_positions.append((x1, y1, x2, y2)) # add split image position
            split_image_info = {"image": image[y1:y2, x1:x2], "coords": (y1, y2, x1, x2)}
            split_images_info.append(split_image_info)

    #print("split_coords: ", split_images_info)

    return split_images_info
    #return split_images, split_positions # return split images and their positions
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset: # path, pre-processing img, origin img, video cap, frame size or path
        ...
        if len(im0.shape) == 3: # check color(width, height, channel)
            rows, cols = a, b  # choose rows and cols
            overlap = 20  # choose overlap pixel
            draw_grid_with_overlap(im0, rows, cols, overlap)
            ...
            for i, det in enumerate(pred):
                ...
                if len(det):
                    for i in range(len(det)):
                        ...
                        det[i, :4] = torch.tensor([x1, y1, x2, y2]).to(det.device)
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            bb_size = int(bb_width * bb_height)
                            #alt = rospy.Subscriber("/mavros/altitude", Altitude, callback)
                            print(f"Class: {class_name}, Size: {bb_size}, Conf: {conf:.2f}")
                            print("Altitude: ", current_altitude)
                            #rospy.Subscriber("/mavros/altitude", Altitude, callback)
                ...
                results.append({'split_info' : split_image_info, 'detections' : det})
